...
BOOL CBackstageWnd::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		CWnd* pWnd;
		BOOL WindowManagement = (FMGetApp()->OSVersion<=OS_Vista) && ((GetKeyState(VK_LWIN)<0) || (GetKeyState(VK_RWIN)<0));
...
INT CBackstageWnd::GetCaptionHeight(BOOL IncludeBottomMargin) const
{
	...
	return max((m_ShowCaption || m_ShowExpireCaption) ? FMGetApp()->m_SmallBoldFont.GetFontHeight()+(IncludeBottomMargin ? 2 : 1)*BACKSTAGECAPTIONMARGIN : 0, Size.cy);
}
		...
		if (m_WantsBitmap || Themed)
		{
			...
			if (Themed)
			{
				...
				if (m_IsDialog)
				{
					...
					Bitmap* pDivider = FMGetApp()->GetCachedResourceImage(IDB_DIVDOWN);
...
void CBackstageWnd::PaintCaption(CPaintDC& pDC, CRect& rect)
{
	if (m_ShowCaption || m_ShowExpireCaption)
	{
		...
		CFont* pOldFont = dc.SelectObject(&FMGetApp()->m_SmallBoldFont);
...
void CBackstageWnd::PrepareBitmaps()
{
	if (!hBackgroundTop)
	{
		Bitmap* pTexture = FMGetApp()->GetCachedResourceImage(IDB_BACKGROUND_DARKLINEN);
...
void CBackstageWnd::OnCompositionChanged()
{
	if (FMGetApp()->m_DwmLibLoaded)
	{
		...
		FMGetApp()->zDwmSetWindowAttribute(m_hWnd, DWMWA_TRANSITIONS_FORCEDISABLED, &ForceDisabled, sizeof(ForceDisabled));

		DWMNCRENDERINGPOLICY NCRP = DWMNCRP_DISABLED;
		FMGetApp()->zDwmSetWindowAttribute(m_hWnd, DWMWA_NCRENDERING_POLICY, &NCRP, sizeof(NCRP));
	}
}
...
BOOL CBackstageWnd::OnCopyData(CWnd* /*pWnd*/, COPYDATASTRUCT* pCopyDataStruct)
{
	...
	FMGetApp()->OpenCommandLine(pCDS->Command[0] ? pCDS->Command : NULL);
