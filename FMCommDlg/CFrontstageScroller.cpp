	...
	: CFrontstageWnd()
{
	...
	m_szScrollStep.cy = FMGetApp()->m_DefaultFont.GetFontHeight();
...
void CFrontstageScroller::DrawNothing(CDC& dc, CRect rect, BOOL Themed) const
{
	...
	rect.top += FMGetApp()->m_DefaultFont.GetFontHeight()+BACKSTAGEBORDER;
}
...
FMFont* CFrontstageScroller::GetLabelFont() const
{
	return &FMGetApp()->m_DefaultFont;
}
