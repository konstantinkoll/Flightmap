...
void FMTooltip::ShowTooltip(const CPoint& point, const CString& strCaption, const CString& strText, HICON hIcon, HBITMAP hBitmap)
{
	...
	if (Themed)
	{
		...
		TextureBrush brush4(FMGetApp()->GetCachedResourceImage(IDB_BACKGROUND_TOOLTIP));
	...
	if (!strCaption.IsEmpty())
	{
		...
		rect.top += FMGetApp()->m_SmallBoldFont.GetFontHeight()+MARGIN;
...
LRESULT FMTooltip::OnNcHitTest(CPoint point)
{
...
void FMTooltip::OnMouseMove(UINT /*nFlags*/, CPoint /*point*/)
{
	HideTooltip();
}
