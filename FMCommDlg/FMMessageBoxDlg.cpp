...
// FMMessageBoxDlg.cpp: Implementierung der Klasse FMMessageBoxDlg
//
...
FMMessageBoxDlg::FMMessageBoxDlg(CWnd* pParentWnd, const CString& Text, const CString& Caption, UINT Type)
	: FMDialog(IDD_MESSAGEBOX, pParentWnd, TRUE)
{
...
BOOL FMMessageBoxDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	return FMDialog::PreTranslateMessage(pMsg);
}

void FMMessageBoxDlg::PaintOnBackground(CDC& dc, Graphics& g, const CRect& rectLayout)
{
...
__forceinline void FMMessageBoxDlg::SetButton(UINT nResID, HINSTANCE hInstance, UINT nCommand, UINT& cButtons)
{
...
BOOL FMMessageBoxDlg::InitDialog()
{
	...
	if (m_Type & MB_ICONMASK)
	{
		...
		{
		case MB_ICONERROR:
			FMGetApp()->PlayErrorSound();
			IconName = IDI_ERROR;

			break;

		case MB_ICONQUESTION:
			FMGetApp()->PlayQuestionSound();
		...
		case MB_ICONSHIELD:
			...
			IconName = (FMGetApp()->OSVersion==OS_Vista) ? MAKEINTRESOURCE(IDI_SHIELD_VISTA) : IDI_SHIELD;
	...
	CDC* pDC = GetDC();
	CFont* pOldFont = pDC->SelectObject(&FMGetApp()->m_DefaultFont);
...
BEGIN_MESSAGE_MAP(FMMessageBoxDlg, FMDialog)
	ON_WM_DESTROY()
	ON_COMMAND_RANGE(IDOK, IDCONTINUE, OnButtonClicked)
END_MESSAGE_MAP()

void FMMessageBoxDlg::OnDestroy()
{
	DestroyIcon(m_hIcon);
...
void FMMessageBoxDlg::OnButtonClicked(UINT nID)
{
	EndDialog(nID);
}
