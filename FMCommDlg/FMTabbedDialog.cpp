...
// FMTabbedDialog.cpp: Implementierungsdatei
//
...
BOOL FMTabbedDialog::AddTab(const CString& Caption)
{
...
BOOL FMTabbedDialog::AddTab(UINT nResID, LPSIZE pszTabArea)
{
	ASSERT(pszTabArea);
...
BOOL FMTabbedDialog::InitSidebar(LPSIZE /*pszTabArea*/)
{
...
BEGIN_MESSAGE_MAP(FMTabbedDialog, FMDialog)
	ON_WM_DESTROY()
...
void FMTabbedDialog::OnRequestTooltipData(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void FMTabbedDialog::OnSelectTab(UINT nCmdID)
{
	SelectTab(nCmdID-IDD_TABBEDDIALOG);
}

void FMTabbedDialog::OnUpdateTabCommands(CCmdUI* pCmdUI)
{
	pCmdUI->Enable();
}
