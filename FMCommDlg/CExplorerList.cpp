...
void CExplorerList::AdjustLayout(INT ListWidth)
{
	...
	if ((IsGroupViewEnabled()) && (FMGetApp()->OSVersion==OS_XP))
		ListWidth -= 14;
...
void CExplorerList::DrawItem(INT nID, CDC* pDC)
{
	...
	case LV_VIEW_TILE:
		...
		const INT FontHeight = FMGetApp()->m_DefaultFont.GetFontHeight();
...
void CExplorerList::ShowTooltip(const CPoint& point)
{
	...
	if (GetOwner()->SendMessage(WM_NOTIFY, tag.hdr.idFrom, LPARAM(&tag)))
		FMGetApp()->ShowTooltip(this, point, tag.Caption[0] ? tag.Caption : GetItemText(m_HoverItem, 0), tag.Hint, tag.hIcon, tag.hBitmap);
}
...
/*void CExplorerList::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		if (m_HoverItem!=m_TooltipItem)
			FMGetApp()->HideTooltip(this);
	}
