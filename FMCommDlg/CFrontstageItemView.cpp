	...
	: CFrontstageScroller(Flags)
{
	...
	// Font height
	m_LargeFontHeight = FMGetApp()->m_LargeFont.GetFontHeight();
	m_DefaultFontHeight = FMGetApp()->m_DefaultFont.GetFontHeight();
	m_SmallFontHeight = FMGetApp()->m_SmallFont.GetFontHeight();
...
{
	const CString className = AfxRegisterWndClass(nClassStyle | CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS, FMGetApp()->LoadStandardCursor(IDC_ARROW));
	...
	for (INT Index=0; Index<m_ItemCount; Index++)
	{
		...
		if (pData->Valid)
		{
			if (GetItemCategory(Index)!=Category)
			{
				...
				lpRect->bottom = (lpRect->top=y)+2*FMCATEGORYPADDING+m_LargeFontHeight;
...
{
	FMSortMemory(m_pItemData, m_ItemCount, m_szData, zCompare, Attr, Descending, Parameter1, Parameter2);
}
...
void CFrontstageItemView::DrawCountItem(CDC& dc, LPCRECT rectItem, INT Index, BOOL Themed, LPCWSTR Label, UINT Count)
{
	...
	{
		if (!m_MaxCountWidth)
			m_MaxCountWidth = FMGetApp()->m_SmallFont.GetTextExtent(_T("888W")).cx+ITEMCELLPADDINGX;
	...
	for (UINT a=0; a<m_ItemCategories.m_ItemCount; a++)
	{
		...
		if (m_ItemCategories[a].IconID)
		{
			...
			if (IntersectRect(&rectIntersect, rectIcon, rectUpdate))
				FMGetApp()->m_CoreImageListJumbo.Draw(&dc, m_ItemCategories[a].IconID-1, rectIcon.TopLeft(), ILD_NORMAL);
		}
	}
}
