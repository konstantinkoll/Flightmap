...
GLuint GLRenderer::CreateTexture(UINT Width, UINT Height, UINT BPP, void* pData, BOOL ForceIntensity, BOOL Repeat, BOOL Mipmap, BOOL Compress) const
{
	...
	Compress &= FMGetApp()->m_TextureCompress && m_SupportsTextureCompression;
...
{
	nResID += min((UINT)m_MaxTextureQuality, (UINT)FMGetApp()->m_TextureQuality);
...
{
	if (m_BlueMarbleCompressed!=FMGetApp()->m_TextureCompress)
	...
	m_BlueMarbleCompressed = FMGetApp()->m_TextureCompress;
}
