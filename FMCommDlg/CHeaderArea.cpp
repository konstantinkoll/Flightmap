...
{
	return 2*BORDER+FMGetApp()->m_CaptionFont.GetFontHeight()+MARGIN+FMGetApp()->m_DefaultFont.GetFontHeight();
}

inline UINT CHeaderArea::GetButtonHeight() const
{
	return m_Buttons.m_ItemCount*(UINT)(FMGetApp()->m_DefaultFont.GetFontHeight()+2*MARGIN+MARGIN/2)-MARGIN/2;
}
...
void CHeaderArea::OnPaint()
{
	...
	if ((m_BackBufferL!=rect.Width()) || (m_BackBufferH!=rect.Height()))
	{
		...
		if (m_RightEdge-LeftEdge>=MINTEXTWIDTH)
		{
			...
			CRect rectText(LeftEdge, BORDER+FMGetApp()->m_CaptionFont.GetFontHeight()+MARGIN, m_RightEdge, rect.bottom-BORDER);
