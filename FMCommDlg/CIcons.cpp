...
INT CIcons::Load(UINT nID, UINT Flags, FMFont* pFont)
{
	if (!pFont)
		pFont = (Flags>=LI_SLIGHTLYLARGER) ? &FMGetApp()->m_LargeFont : &FMGetApp()->m_DefaultFont;
	...
	static const INT IconSizes[] = { 16, 24, 32, 48, 64 };
	const INT Size = IconSizes[Level];
